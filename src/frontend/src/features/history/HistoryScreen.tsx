...
import { deleteSession, getSessionById, listSessions } from "../../api/sessions";
import { ApiError } from "../../api/client";
import type { SessionDetail, SessionMetadata } from "../../types/session";
import SensorGrid from "../telemetry/components/SensorGrid";

const formatDate = (iso: string): string => new Date(iso).toLocaleString("pt-BR");
...
export default function HistoryScreen() {
  ...
  const loadSessions = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Falha ao carregar sessões.");
    } finally {
  ...
  const handleSelectSession = async (id: string) => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Falha ao carregar detalhes.");
    } finally {
  ...
  const calculatedStats = useMemo(() => {
    ...
    const calcPct = (v: number | null) => {
      if (!v || v === 0) return 0;
      return Math.max(0, Math.min(100, Math.round(((v - 9.9) / (12.6 - 9.9)) * 100)));
    };
    const energyDelta = Math.max(0, calcPct(selectedSession.initialVoltage) - calcPct(selectedSession.finalVoltage));
    const durationSeconds = (selectedSession.durationMs || 0) / 1000;
    const speed = durationSeconds > 0 ? `${((selectedSession.steps.length * 18) / durationSeconds).toFixed(1)} cm/s` : "15.4 cm/s";
      ...
      <main className="w-full h-full p-6 flex flex-col gap-4 overflow-hidden font-mono text-xs">
        ...
        <div className="flex items-center justify-between shrink-0 border-b border-outline-variant/20 pb-4">
          <div className="flex items-center gap-4">
            <button
              onClick={() => { setIsReplaying(false); setSelectedSession(null); setReplayIndex(0); }}
          ...
          <span className={`px-3 py-1 border text-[10px] uppercase tracking-widest ${selectedSession.completed ? "border-emerald-500/40 text-emerald-400 bg-emerald-500/10" : "border-amber-500/40 text-amber-400 bg-amber-500/10"}`}>
            ...
            <div className="bg-surface-container-low/60 border border-outline-variant/30 p-4 shrink-0">
              ...
              <div className="grid grid-cols-2 gap-3 text-[10px]">
                <div className="bg-surface-container-lowest p-2 border border-outline-variant/10">
                  ...
                  <span className="font-bold text-on-surface">{formatDuration(selectedSession.durationMs)}</span>
                </div>
                <div className="bg-surface-container-lowest p-2 border border-outline-variant/10">
                  <span className="text-outline block mb-1">Velocidade</span>
                  <span className="font-bold text-on-surface">{calculatedStats.averageSpeed}</span>
                </div>
                <div className="bg-surface-container-lowest p-2 border border-outline-variant/10">
                  <span className="text-outline block mb-1">Consumo Bat.</span>
                  <span className="font-bold text-emerald-400">{calculatedStats.batteryUsage}</span>
                </div>
                <div className="bg-surface-container-lowest p-2 border border-outline-variant/10">
                ...
                <div className="col-span-2 bg-surface-container-lowest p-2 border border-outline-variant/10">
                  <span className="text-outline block mb-1">Variação de Tensão (Início &rarr; Fim)</span>
                  <span className="font-bold text-on-surface">{formatVoltage(selectedSession.initialVoltage)} &rarr; {formatVoltage(selectedSession.finalVoltage)}</span>
                </div>
              </div>
            </div>
              ...
              {replayStep ? (
                <div className="grid grid-cols-2 gap-3 text-[10px]">
                  <div className="bg-surface-container-lowest p-2 border border-outline-variant/10">
                     ...
                     <span className="text-primary font-bold">{replayStep.voltage.toFixed(2)}</span>
                  </div>
                  <div className="bg-surface-container-lowest p-2 border border-outline-variant/10">
                     <span className="text-outline block">Corrente (mA)</span>
                     <span className="text-primary font-bold">{replayStep.current}</span>
                  </div>
                  <div className="col-span-2 bg-surface-container-lowest p-2 border border-outline-variant/10">
                     <span className="text-outline block">Timestamp do Passo</span>
                     <span className="text-on-surface font-bold">{replayStep.createdAt ? new Date(replayStep.createdAt).toLocaleTimeString("pt-BR") : "—"}</span>
                  </div>
                </div>
              ) : (
            ...
            <div className="bg-surface-container-low/60 border border-outline-variant/30 p-4 mb-4 shrink-0">
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-outline w-8">{stepCount > 0 ? stepCount - 1 : 0}</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-2">
                    ...
                    <button 
                      onClick={() => { setIsReplaying(false); setReplayIndex(0); }} 
              ...
              {isLoadingDetail ? (
                ...
                <div className="w-full max-h-full aspect-square relative">
                  <div className="absolute top-2 left-2 z-10 bg-black/60 border border-outline-variant/20 px-2 py-1 text-[10px] text-outline backdrop-blur-sm">
                    Posição Robô: <span className="text-primary font-bold">X:{replayStep?.posX} Y:{replayStep?.posY}</span>
                  </div>
                  <SessionReplayGrid
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {sessions.map((session) => (
                ...
                >
                  <td className="px-4 py-4 text-primary font-bold group-hover:underline">{session.name}</td>
                  <td className="px-4 py-4 text-on-surface-variant">{session.algorithm}</td>
                  <td className="px-4 py-4 text-outline">{formatDate(session.createdAt)}</td>
                  <td className="px-4 py-4 text-on-surface">{formatDuration(session.durationMs)}</td>
                  <td className="px-4 py-4 text-center">
                    <span className={`text-[9px] uppercase px-2 py-0.5 border ${session.completed ? "border-emerald-500/40 text-emerald-400 bg-emerald-500/5" : "border-amber-500/40 text-amber-400 bg-amber-500/5"}`}>
