...
void registrarParede(Labirinto *lab, int x, int y, char direcao) {
    switch (direcao) {
        case 'N':
            ...
            if (y+1 < LAB_TAM) lab->celula[x][y+1].sul   = true;
            break;
        case 'S':
            lab->celula[x][y].sul = true;
            if (y-1 >= 0)       lab->celula[x][y-1].norte = true;
            break;
        case 'L':
            lab->celula[x][y].leste = true;
            if (x+1 < LAB_TAM) lab->celula[x+1][y].oeste = true;
            break;
        case 'O':
            lab->celula[x][y].oeste = true;
            if (x-1 >= 0)       lab->celula[x-1][y].leste = true;
            break;
    }
}