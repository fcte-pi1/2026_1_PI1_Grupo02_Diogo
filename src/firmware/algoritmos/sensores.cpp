...
void inicializaSensores(uint8_t trigFrente,   uint8_t echoFrente,
    ...
    _trigF = trigFrente;   _echoF = echoFrente;
    _trigE = trigEsquerda; _echoE = echoEsquerda;
    _trigD = trigDireita;  _echoD = echoDireita;

    pinMode(_trigF, OUTPUT); digitalWrite(_trigF, LOW);
    pinMode(_trigE, OUTPUT); digitalWrite(_trigE, LOW);
    pinMode(_trigD, OUTPUT); digitalWrite(_trigD, LOW);
...
void atualizaSensores() {
    ...
    if (agora - ultimoTrigger < INTERVALO_TRIGGER_MS) return;
    ultimoTrigger = agora;

    switch (vez) {
        case 0: _dispararTrigger(_trigF); break;
        case 1: _dispararTrigger(_trigE); break;
        case 2: _dispararTrigger(_trigD); break;
    }
    vez = (vez + 1) % 3;
}
