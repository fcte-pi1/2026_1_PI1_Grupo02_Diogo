...
void inicializaMotors(uint8_t in1L, uint8_t in2L,
    ...
    _in1L   = in1L;   _in2L   = in2L;
    _in1R   = in1R;   _in2R   = in2R;
    _encEsq = encEsq; _encDir = encDir;

    pinMode(_in1L, OUTPUT); pinMode(_in2L, OUTPUT);
    pinMode(_in1R, OUTPUT); pinMode(_in2R, OUTPUT);

    // motors parados no cmc
    digitalWrite(_in1L, LOW); digitalWrite(_in2L, LOW);
    digitalWrite(_in1R, LOW); digitalWrite(_in2R, LOW);
}
...
static void _fowardMotors() {
    digitalWrite(_in1L, HIGH); digitalWrite(_in2L, LOW);
    digitalWrite(_in1R, HIGH); digitalWrite(_in2R, LOW);
}

// Curva a esquerda
static void _rotateLeftMotors() {
    digitalWrite(_in1L, LOW);  digitalWrite(_in2L, HIGH);
    digitalWrite(_in1R, HIGH); digitalWrite(_in2R, LOW);
}
...
static void _rotateRightMotors() {
    digitalWrite(_in1L, HIGH); digitalWrite(_in2L, LOW);
    digitalWrite(_in1R, LOW);  digitalWrite(_in2R, HIGH);
}


static void _esperarEncoder(long pulsos) {  // Pra saber quando ele andou o suficiente(1 celula, 90° ou 180°)
...
static int _indiceDirecao(char d) {
    for (int i = 0; i < 4; i++) if (_dirs[i] == d) return i;
    return 0;
}
